...
class ProcessingTracker {
    constructor() {
        ...
        this.completedRollNumbers = new Set();
    }
    ...
    finishProcessing(roll_number, success) {
        this.processingRollNumbers.delete(roll_number);
        if (success) {
            this.completedRollNumbers.add(roll_number);
        }
    }

    getCompletedRollNumbers() {
        return Array.from(this.completedRollNumbers);
    }
}
...
async function processRollNumber(roll_number) {
    ...
    try {
        ...
        processingTracker.finishProcessing(roll_number, success);
        semaphore.release();
    }
}
...
function generateYearList(prefix) {
    ...
    if(prefix === "17") {
        ...
        yearList = ["2004", "2003", "2005", "2002", "2001", "2006", "2007", "2000", "1999", "2008", "1998"]
    }
    else if(prefix === "22") {
        yearList = ["2005", "2004", "2006", "2003", "2002", "2007", "2008", "2001", "2000", "2009", "1999"]
    }
    else if(prefix === "23") {
...
async function main() {
    while (true) {
        try {
            // Get 8 pending students that aren't currently being processed
            const students = await prisma.student.findMany({
                ...
                take: 12 - processingTracker.processingRollNumbers.size,
            });

            if (students.length === 0 && processingTracker.processingRollNumbers.size === 0) {
            ...
            const processingPromises = students.map(student => 
            ...
            // If any student's password was found, immediately start processing another
            if (processingTracker.getCompletedRollNumbers().length > 0) {
                continue; // This will trigger a new iteration to fill processing slots
            }
